import { VillageStatisticItem } from '@/app/infographics/_constants/village-statistics-data';

interface StatisticCardProps {
	statistic: VillageStatisticItem;
}

export const VillageStatisticCard = ({ statistic }: StatisticCardProps) => {
	...
	return (
		<div className="group relative overflow-hidden rounded-xl border border-slate-200/80 bg-white p-5 shadow-sm transition-all duration-200 hover:border-slate-300 hover:shadow-md">
			...
			<div className="mt-4 flex flex-col gap-0.5">
				<p className="text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">{statistic.value}</p>

				<p className="text-sm font-medium text-slate-600 leading-snug">{statistic.label}</p>
			</div>

			<div className="mt-4 pt-3 border-t border-slate-100">
				<p className="text-xs text-slate-500 leading-relaxed">{statistic.description}</p>
			</div>
		</div>
	);
};
